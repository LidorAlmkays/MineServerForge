...
type stackedCustomLogger struct {
	prefix string
	mode   enums.ProgramMode
}

func NewStackedCustomLogger(mode enums.ProgramMode, prefix string) Logger {
	return &stackedCustomLogger{mode: mode, prefix: prefix}
}

// Info prints the message in yellow to the console with a timestamp
func (l *stackedCustomLogger) Info(msg string) {
	if l.mode != enums.ProductionMode {
...
func (l *stackedCustomLogger) Debug(msg string) {
	if l.mode == enums.DevelopmentMode {
